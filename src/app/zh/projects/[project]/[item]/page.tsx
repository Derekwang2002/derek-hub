...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return getProjectMetadata(project, item, "zh");
}

export default async function ChineseProjectDocumentRoute({ params }: Props) {
  const { project, item } = await params;
  return <ProjectItemPage itemSlug={item} locale="zh" projectSlug={project} />;
}
