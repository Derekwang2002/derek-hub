...
如果还不清楚 CALL-E 与桌面 Agent、Bridge、浏览器能力之间的关系，可以先看[《CALL-E 系统总览》](/zh/blog/call-e-overview)。
...
### 12.1 RunGroup 与 Call：不要把执行层的 ID 混为一谈

`RunGroup` 是单次或批量执行的组织单元：一个 Goal 可以有多个 RunGroup，一个 RunGroup 又可以包含多个 Run。它负责把同一轮业务执行归在一起，并不替代 Run 的单次尝试语义。

对外呼场景，`Call` 也不是与 Goal、Run 并列的产品核心实体，而是 voice 类型 Run 在外部通话系统留下的记录。因此需要明确区分三类标识：

| 标识 | 所在层 | 用途 |
|---|---|---|
| `Run ID` | CALL-E | 一次可审计的业务执行尝试 |
| Calling `task_id` / `external_run_id` | Calling Runtime | 外部执行任务及其异步状态 |
| Provider `call_id` | 电话 Provider | 某一通具体电话的记录 |

一次 Run 可能先创建 Calling task，随后才产生 provider call；重试则会创建新的 Run 或新的外部任务。排查问题时，应先以 Run 为业务锚点，再沿外部 ID 查到具体通话，而不是把任意一个 `call_id` 当成整个 Goal 的身份。

### 12.2 完整外呼链路：谁决策，谁执行

把完整链路按职责拆开，可避免把模型决策、持久化状态和真实拨号混成一个同步调用：

1. **MainAgent** 收到用户目标，补齐阻塞信息，生成 GoalBrief，并用 `commit_goal` 写入 Goal 与初始 Event。
2. **Goal Runtime** 取得 Lease、读取 Cursor 之后的新 Event，恢复 GoalAgent Session 并启动一轮 GoalAgent。
3. **GoalAgent** 决定下一步：生成或更新 RunSpec；需要授权时通过 Context Delivery 请求 MainAgent 向用户确认。
4. 用户确认后，**GoalAgent** 调用 `submit_voice_run`，只创建 `queued` 的 Run；本轮 Iteration 结束，不等待拨号。
5. **VoiceRunExecutor / Calling Runtime** 认领 Run，创建外部 task，实际拨号，并把状态、Transcript、结果和证据回写为 Run Event 与 Goal Event。
6. 新 Event 再次唤醒 **GoalAgent**；它依据成功条件决定重试、继续、失败或完成，必要时生成并 `commit_report`。
7. **Context Delivery** 将结构化结果交回 MainAgent；只有 MainAgent 面向用户组织最终回复。

这条链路的关键边界是：GoalAgent 决定业务下一步，Runtime 负责可靠落库与调度，Voice Runtime 负责现实世界副作用，MainAgent 负责用户对话。任何一个组件重启，都不应依赖另一组件的内存才能恢复任务。
