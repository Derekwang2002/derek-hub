...
本文基于 `s-eleven-mono` 提交 `b36ac02f`。我们不只看一个函数，而是沿着一次真实工具调用，追踪 `commit_goal` 如何把对话中的目标变成可持久化、可重试、可交给后台继续执行的系统状态。
...
源码会继续变化，所以文件路径和符号名比本文中的瞬时行号更值得记忆。本文所有结论都以提交 `b36ac02f` 为准。
