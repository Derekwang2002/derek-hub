...
This article is based on `s-eleven-mono` commit `b36ac02f`. Instead of reading a single function in isolation, we will follow one real tool invocation and see how `commit_goal` turns a conversational objective into system state that can be persisted, retried, and handed to background execution.
...
Source code continues to evolve, so file paths and symbol names are more useful than transient line numbers. Every conclusion in this article is based on commit `b36ac02f`.
